      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {(showPresent ? facultyData.present : facultyData.retired).map((member) => (
          <div key={member.name} className={styles.facultyCard}>
            ...
            <p className={styles.facultyPosition}>{member.position}</p>
            <p className={styles.facultyResearch}>
              <strong>Research Interest:</strong> {member.researchInterest}
            </p>
            <p className={styles.facultyEmail}>
              <EmailIcon className="to-black" />: <a href={`mailto:${member.email}`} className="text-blue-500 underline">{member.email}</a>
            </p>
            <p className={styles.facultyPhone}><PhoneIcon />: {member.phone}</p>
            <p className={styles.facultyJoined}>Joined: {member.joined}</p>
            <NavLink to={`/faculty/${member.id}`} className={styles.detailsButton}>
              View Details
            </NavLink>
          </div>
        ))}
